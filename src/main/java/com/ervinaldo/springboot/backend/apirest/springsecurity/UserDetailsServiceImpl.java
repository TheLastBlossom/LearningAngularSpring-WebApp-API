...
import com.ervinaldo.springboot.backend.apirest.models.entity.CRol;
...
public class UserDetailsServiceImpl implements UserDetailsService {
	...
	public UserDetails loadUserByUsername(String correo) throws UsernameNotFoundException {
		com.ervinaldo.springboot.backend.apirest.models.entity.User user = usuariodao.findByUsername(correo);
		if (user == null) {
			throw new UsernameNotFoundException(correo);
		}
		List<String> rolesAsString = user.getRols().stream().map(CRol::getRol) // Obtiene el nombre del rol como cadena
